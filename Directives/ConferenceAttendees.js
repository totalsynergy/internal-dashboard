...
app.directive("conferenceAttendees", function(){
    ...
    var controller = function($scope, EventbriteService, Service, ngAudio){
    $scope.totalAttendees = 0;
    $scope.sound = ngAudio.load("assets/bell.mp3");
    $scope.count = 0;

    $scope.$on('attendeesUpdated', function() {
      if($scope.totalAttendees != EventbriteService.totalAttendees && $scope.totalAttendees != 0){
        Service.updateTab(3, function(){
          $scope.sound.play();
        });
      }
      $scope.totalAttendees = EventbriteService.totalAttendees;
