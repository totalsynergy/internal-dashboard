var Heatmap = function(width, container) {
    Heatmap.width = width;
    Heatmap.height = Heatmap.width / 2.5;
    Heatmap.scale = Heatmap.width / 1000;
    Heatmap.ngroups = 100;
    ...
    Heatmap.init = function(markedCallback) {
        Heatmap.markedCallback = markedCallback;

        $(document).keydown(function(e) {
            Heatmap.shifthold = e.shiftKey;
        });
        ...
        Heatmap.stage.on('mouseup', function() {
            if (Heatmap.getMode() === 'single' && Heatmap.emptyclick && !Heatmap.shifthold) {
                Heatmap.marked = new Array();

                Heatmap._handleMarkedCountries(Heatmap.marked);
            }

            Heatmap.emptyclick = true;
        });

        Heatmap.mapLayer = new Kinetic.Layer({
            scale: Heatmap.scale
        });

        Heatmap.selectedLayer = new Kinetic.Layer({
            scale: Heatmap.scale
        });

        Heatmap.rectLayer = Heatmap._initSelectRectangle();

        Heatmap.marked = new Array();

        Heatmap._drawMap();

        Heatmap._addCountryClickAction();

        Heatmap.stage.add(Heatmap.mapLayer);
        Heatmap.stage.add(Heatmap.selectedLayer);
        Heatmap.stage.add(Heatmap.rectLayer);

        Heatmap.stage.draw();
    };

    Heatmap.updateMap = function(values) {
        var max = Heatmap._getMaxValue(values);

        for (var child in Heatmap.mapLayer.getChildren()) {
            var key = Heatmap.mapLayer.getChildren()[child].getId();
            ...
            Heatmap.mapLayer.getChildren()[child].setFill(rgb);
        }

        Heatmap.mapLayer.draw();
    };

    Heatmap.setMode = function(mode) {
        Heatmap.mode = mode;

        if (mode === 'single') {
            Heatmap.rectLayer.setZIndex(-1);
            Heatmap.mapLayer.setZIndex(1);
            Heatmap.selectedLayer.setZIndex(2);
        }
        else if (mode === 'multi') {
            Heatmap.mapLayer.setZIndex(1);
            Heatmap.selectedLayer.setZIndex(2);
            Heatmap.rectLayer.setZIndex(3);
        }

        if (Heatmap.rect !== undefined) {
            Heatmap.rect.remove();
            Heatmap.rectLayer.draw();
        }
    };

    Heatmap.getMode = function() {
        return Heatmap.mode;
    };

    Heatmap._drawMap = function() {
        for (var key in Heatmap.worldmap.shapes) {
            var path = new Kinetic.Path({
                data: Heatmap.worldmap.shapes[key],
                stroke: Heatmap.borderStroke,
                strokeWidth: Heatmap.borderStrokeWidth,
                id: key
            });

            Heatmap.mapLayer.add(path);

            path.boxes = new Array();

            var points = Kinetic.Path.parsePathData(path.getData());
            ...
            for (var i = 0; i < points.length; i++) {
                if (points[i]['points'].length === 2) {
                    minx = Math.min(minx, points[i]['points'][0] * 1);
                    miny = Math.min(miny, points[i]['points'][1] * 1);
                    maxx = Math.max(maxx, points[i]['points'][0] * 1);
                    maxy = Math.max(maxy, points[i]['points'][1] * 1);
                }
                else {
                    path.boxes.push([minx, maxx, miny, maxy]);
    ...
    Heatmap._addCountryClickAction = function() {
        for (var child in Heatmap.mapLayer.getChildren()) {
            var path = Heatmap.mapLayer.getChildren()[child];

            path.on('mousedown', function(evt) {
                Heatmap.selectedLayer.removeChildren();

                if (Heatmap.getMode() !== 'single') {
                    return;
                }

                if (!Heatmap.shifthold) {
                    Heatmap.marked = new Array();
                }

                for (var child in Heatmap.mapLayer.getChildren()) {
                    var co = Heatmap.mapLayer.getChildren()[child];

                    if (this.getId() === co.getId()) {

                        if (!Heatmap.shifthold) {
                            Heatmap.marked.push(co);
                        }
                        else {
                            var existing = false;
                            for (var i = 0; i < Heatmap.marked.length; i++) {
                                if (co.getId() === Heatmap.marked[i].getId()) {
                                    Heatmap.marked.splice(i, 1);
                                    existing = true;
                                    break;
                                }
                            }
                ...
                Heatmap.emptyclick = Heatmap.marked.length === 0;

                Heatmap._handleMarkedCountries(Heatmap.marked);

                Heatmap.selectedLayer.draw();
            });
        }
    };

    Heatmap._getMousePosition = function(evt) {
        return {
            x: (evt.offsetX || evt.layerX) / Heatmap.scale,
            y: (evt.offsetY || evt.layerY) / Heatmap.scale
        };
    };

    Heatmap._initSelectRectangle = function() {
        var down = false;
        ...
        rectLayer.add(new Kinetic.Rect({
            x: 0,
            y: 0,
            width: Heatmap.width,
            height: Heatmap.height
        ...
        rectLayer.on("mousedown", function(e) {
            if (Heatmap.getMode() !== 'multi') {
                return;
            }
            ...
            var offset = Heatmap._getMousePosition(e);

            down = true;

            Heatmap.rect = new Kinetic.Rect({
                x: offset.x * Heatmap.scale,
                y: offset.y * Heatmap.scale,
                width: 1,
                height: 1,
                stroke: Heatmap.rectStroke,
                strokeWidth: Heatmap.rectStrokeWidth
            });

            rectLayer.add(Heatmap.rect);
        });

        rectLayer.on("mousemove", function(e) {
            if (!down)
                return;

            var offset = Heatmap._getMousePosition(e);

            var p = Heatmap.rect.attrs;

            if (Heatmap.getMode() === 'multi') {
                Heatmap.rect.setWidth((offset.x * Heatmap.scale) - p.x);
                Heatmap.rect.setHeight((offset.y * Heatmap.scale) - p.y);
            }

            rectLayer.draw();
        });

        rectLayer.on("mouseup", function(e) {
            down = false;

            if ((Heatmap.getMode() !== 'multi') || Heatmap.rect === undefined) {
                return;
            }

            var x1 = Heatmap.rect.getPosition().x / Heatmap.scale;
            var x2 = (Heatmap.rect.getPosition().x / Heatmap.scale) + (Heatmap.rect.getWidth() / Heatmap.scale);

            var y1 = Heatmap.rect.getPosition().y / Heatmap.scale;
            var y2 = (Heatmap.rect.getPosition().y / Heatmap.scale) + (Heatmap.rect.getHeight() / Heatmap.scale);
            ...
            if (Heatmap.getMode() === 'multi') {
                ...
                for (var child in Heatmap.mapLayer.getChildren()) {
                    ...
                    for (var i = 0; i < co.boxes.length; i++) {
                        if (co.boxes[i][0] >= x1 && co.boxes[i][1] <= x2 && co.boxes[i][2] >= y1 && co.boxes[i][3] <= y2) {
                            hits++;
                        }
                    }

                    if (hits === co.boxes.length) {
                        Heatmap.marked.push(co);
                        count++;
                    }
                }
    ...
    Heatmap._getMaxValue = function(array) {
        var max = -1;

        for (var key in array) {
            max = Math.max(max, array[key]);
        }

        return max;
    };

    Heatmap._handleMarkedCountries = function(arr) {
        Heatmap._shadeUnmarked(arr);

        if (Heatmap.markedCallback !== undefined && Heatmap.markedCallback !== null) {
            Heatmap.markedCallback(arr);
        }
    };

    Heatmap._shadeUnmarked = function(arr) {
        function isMarked(co, arr) {
            for (var i = 0; i < arr.length; i++) {
                if (co.getId() === arr[i].getId()) {
                    return true;
                }
            }
        ...
        for (var child in Heatmap.mapLayer.getChildren()) {
            ...
            if (!isMarked(co, arr)) {
                var shade = co.clone();
                shade.setFill(Heatmap.shadeFill);
                shade.setOpacity(Heatmap.shadeOpacity);
                Heatmap.selectedLayer.add(shade);
            }
        }
